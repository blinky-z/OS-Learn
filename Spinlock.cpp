...
void Spinlock::acquire() {
    ...
    // --------------------
    // atomic<bool> переменная не подходит, так как ее реализация -
    // lock-free, или же с использованием лока - зависит от компилятора и операционной системы, тогда как atomic_flag
    // гарантированно реализован lock_free, это задокументировано стандартом, так что мы можем быть уверены что
    // спинлок реализованный с помощью std::atomic_flag будет работать правильно (то есть не блокируя поток) на любой
